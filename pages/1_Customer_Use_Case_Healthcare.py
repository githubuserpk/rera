...
def display_title():
    st.title("Customer Use Case - Healthcare provider based in UK")
...
def main():
    st.sidebar.title("Customer Use Case - Healthcare provider based in UK")
    ...
    with st.container() as main_container:
        ...
        selected_compRev = display_radio_button(compRev, "Annual Revenue:", b_horizontal=True, key="compRev_selector",index=len(compRev)-1)
        ...
        selected_roletype, cust_role = display_adv_radio_button(roles, "Do you know your role:", b_horizontal=True, key="roletype_selector", index=1)
        ...
        sic_desc = "Agents specialized in the sale of other particular products and Other education n.e.c. and General medical practice activities and Specialists medical practice activities"
        query = f"""Nature of business is {selected_nob} and 
                using {selected_series} and 
                using model {model_name} and 
                for {selected_custtype} and 
                doing {finetune_type} 
                with {ft_technique_type} 
                and the entity is involved in the following businesses {sic_desc}.
                The entity has annual revenues of {selected_compRev}
                 """
